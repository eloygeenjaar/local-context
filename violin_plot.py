...
dfnc = np.load('results/dfnc.npy')

print(proposed_arr[:, 1, 0].shape, lvae[:, 0, 0].shape)
results = np.stack((proposed_arr[:, 1, 0], lvae[:, 0, 0], np.ones((proposed_arr.shape[0], )) * dfnc[0]), axis=-1)
print(ttest_ind(proposed_arr[:, 1, 0], lvae[:, 0, 0]))
fig, ax = plt.subplots(1, 1)
bplot = ax.boxplot(results, labels=['Local-global', 'Local only', 'dFNC'], patch_artist=True)
ax.set_ylabel('Window classification accuracy')
print(proposed_arr, lvae)
